...
public class Vect {
    ...
    public int crossSgn(Vect v) {
        BigDecimal c = this.cross(v);
        return Arithmetic.sgn(c);
    }
    ...
    public BigDecimal interpolationYmid() {
        BigDecimal r = new BigDecimal("0.5");
        BigDecimal y = from.getY().multiply(r).add(to.getY().multiply(BigDecimal.ONE.subtract(r)));
        return Point.interpolationY(from, to, y).getY().add(BigDecimal.ZERO, Arithmetic.MATH_CONTEXT);
    }
